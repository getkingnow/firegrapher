var FireGrapher = (function() {
      ...
      case "scatter":
        this.firebaseRef.child(path).on(eventToListenTo, function(childSnapshot) {
          ...
          if (typeof this.config.xCoord.stream !== "undefined" && this.config.xCoord.stream) {
            var xCoord = (this.streamCounts[series]) ? this.streamCounts[series] : 0;
          }
          else {
            var xCoord = parseInt(data[this.config.xCoord.value]);
          }
  ...
  FireGrapher.prototype.addDataPointToTable = function(newDataPoint) {
    ...
    var rows = this.outerTable.selectAll("div.row").data(this.tableRows).enter().append("div").attr("class", "row clearfix");
    ...
    switch (graphType) {
      case "line":
        ...
        if (typeof line === "undefined") {
          ...
          this.drawCircleForDataPoint(newDataPoint);
        }
        break;
      case "scatter":
        this.drawCircleForDataPoint(newDataPoint);
  ...
  FireGrapher.prototype.drawCircleForDataPoint = function(newDataPoint) {
