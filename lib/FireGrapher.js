var FireGrapher = (function() {
    ...
    this.parsePathNode(["/"], nodes, 0).then(function(paths) {
      ...
      paths.forEach(function(path) {
        ...
        switch (this.config.type) {
          case "table":
            this.firebaseRef.child(path).on(this.dataRetrievalEvent, function(childSnapshot) {
          ...
          case "line":
            this.firebaseRef.child(path).on(this.dataRetrievalEvent, function(childSnapshot) {
              ...
              if (typeof this.config.xCoord.stream !== "undefined" && this.config.xCoord.stream) {
                if (typeof this.graphData[line] === "undefined") {
                  this.graphData[line] = {
                    coordinates : []
                  };
                }
                xCoord = this.graphData[line].coordinates.length;
              }
              else {
              ...
              console.log("LOGGING");
              console.log(xCoord);
              console.log(data[this.config.yCoord.value]);
  ...
  FireGrapher.prototype.parsePathNode = function(paths, nodes, nodeIndex) {
    return new RSVP.Promise(function(resolve) {
      ...
      if (node[0] === "*") {
        ...
        this.dataRetrievalEvent = "child_added";
        resolve(paths);
      }
  ...
  FireGrapher.prototype.addDataPointToLineGraph = function(newDataPoint) {
    ...
    this.graphData[newDataPoint.line].coordinates.push({ x : newDataPoint.xCoord, y : newDataPoint.yCoord });
    if (newDataPoint.xCoord >
        this.graphData[newDataPoint.line].coordinates[this.graphData[newDataPoint.line].coordinates.length - 2]) {
      // need to sort because x coords are now out of order (so that our line doesn't plot backwards)
      this.graphData[newDataPoint.line].coordinates.sort(function(a, b) { return a.x - b.x; });
    }
    ...
    if (typeof line === "undefined") {
      line = this.addLineToGraph(newDataPoint.line, this.graphData[newDataPoint.line].coordinates);
    }
    else {
      // update the line with the data
      this.graph.select(this.cssSelector + " path." + newDataPoint.line)
        .data([this.graphData[newDataPoint.line].coordinates])
        .attr("d", line)
        ;
  ...
  FireGrapher.prototype.renderGraph = function() {
    ...
    // removing old scales
    while (this.graph.select(this.cssSelector + " g").remove()[0][0] !== null) {
      continue;
    }
  ...
  FireGrapher.prototype.addLineToGraph = function(name, data) {
    ...
    // adding points
    this.graph.selectAll(this.cssSelector + " circle." + name)
      .data(data)
      .enter().append("circle")
      .attr("class", "dot " + name)
      .attr("cx", line.x())
