var FireGrapher = (function() {
  ...
  FireGrapher.prototype.drawLineGraph = function() {
    ...
    var graphWidth = (this.config.graph && this.config.graph.width) ? this.config.graph.width : 500;
    var graphHeight = (this.config.graph && this.config.graph.height) ? this.config.graph.height : 150;
    var minX = (this.config.xCoord.min) ? this.config.xCoord.min : 0;
    var maxX = (this.config.xCoord.max) ? this.config.xCoord.max : 50;
    var minY = (this.config.yCoord.min) ? this.config.yCoord.min : 0;
    var maxY = (this.config.yCoord.max) ? this.config.yCoord.max : 200;

    this.graphData = {};
    this.graphLines = {};
    this.interpolateXCoord = d3.scale.linear().domain([minX, maxX]).range([0, graphWidth]);
    this.interpolateYCoord = d3.scale.linear().domain([minY, maxY]).range([graphHeight, 0]);
