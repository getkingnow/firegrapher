...
FireGrapher.prototype.addDataPointToBarGraph = function(newDataPoint) {
  ...
  switch(aggtype) {
    case "mean":
      var sum = 0;
      for (; i < this.graphData[newDataPoint.series].values.length; i++) {
        sum += this.graphData[newDataPoint.series].values[i];
      }
      this.graphData[newDataPoint.series].aggregation = sum / this.graphData[newDataPoint.series].values.length;
      break;
    case "median":
      var tmpArray = this.graphData[newDataPoint.series].values.slice(0); // slice will clone
      tmpArray.sort(function(a, b) { return a-b; });
      this.graphData[newDataPoint.series].aggregation = tmpArray[Math.ceil(tmpArray.length / 2)];
      break;
    case "min":
      this.graphData[newDataPoint.series].aggregation = Number.MAX_VALUE;
      for (; i < this.graphData[newDataPoint.series].values.length; i++) {
        if (this.graphData[newDataPoint.series].values[i] < this.graphData[newDataPoint.series].aggregation) {
          this.graphData[newDataPoint.series].aggregation = this.graphData[newDataPoint.series].values[i];
        }
      }
      break;
    case "max":
      this.graphData[newDataPoint.series].aggregation = Number.MIN_VALUE;
      for (; i < this.graphData[newDataPoint.series].values.length; i++) {
        if (this.graphData[newDataPoint.series].values[i] > this.graphData[newDataPoint.series].aggregation) {
          this.graphData[newDataPoint.series].aggregation = this.graphData[newDataPoint.series].values[i];
        }
      ...
      break;
    case "sum":
      this.graphData[newDataPoint.series].aggregation += newDataPoint.value;
      break;
    default: // default sum
      this.graphData[newDataPoint.series].aggregation += newDataPoint.value;
      break;
  }
  this.graphData[newDataPoint.series].aggregation = (this.graphData[newDataPoint.series].aggregation) ? this.graphData[newDataPoint.series].aggregation : 0;
  ...
  redrawGraph = redrawGraph || this.changeScales(
    ...
    [0, this.graphData[newDataPoint.series].aggregation]);
  if (redrawGraph) {
...
FireGrapher.prototype.drawLegend = function() {
  var series = [];
  ...
  // remove old legend
  this.graph.selectAll("g.fg-legend").remove();
  // if multiple series, draw new legend
  if (series.length > 1) {
    var margin = { top: 5, bottom: 5, left: 5, right: 5 };
    var legendWidth = 50;
    var legendHeight = series.length * 20;
    var x = this.config.styles.size.width - legendWidth * 2 - margin.left - margin.right;
    var y = this.config.styles.size.height - legendHeight * 2 - margin.top - margin.bottom;
    ...
    gs.selectAll("text")
      ...
      .append("text")
        ...
        .style("stroke", function(d, i) {
          return this.config.styles.series.strokeColors[i];
        }.bind(this))
        .text(function(d, i) {
          return d;
        });
...
FireGrapher.prototype.drawLine = function(seriesIndex, dataPoints) {
  ...
  if (this.graph.selectAll("path.fg-series-" + seriesIndex)[0].length === 0) {
    this.graph
      .append("path")
  ...
  this.graph.select("path.fg-series-" + seriesIndex)
    .data([dataPoints])
    .attr("class", "fg-series fg-series-" + seriesIndex)
...
FireGrapher.prototype.drawDataPoints = function(seriesIndex, dataPoints) {
  this.graph.selectAll("circle.fg-series" + seriesIndex)
...
FireGrapher.prototype.drawBar = function(series, barData) {
  ...
  this.graph.selectAll(".fg-bar .fg-series"+seriesIndex)
    .data([barData]).enter()
    .append("rect")
      .attr("class", "fg-series fg-series-" + seriesIndex)
      ...
      .attr("y", function(d) { return this.yScale(d.aggregation); }.bind(this))
      .attr("height", function(d) { return this.yScale.range()[0] - this.yScale(d.aggregation); }.bind(this));
};
...
FireGrapher.prototype.drawGraph = function() {
  ...
  this.graph = d3.select(this.cssSelector)
    .append("svg")
      .attr("class", "fg-" + this.config.type)
  ...
  this.graph.selectAll(".fg-x-axis .tick")
    .attr("stroke", this.config.styles.innerStrokeColor)
    .attr("stroke-width", (this.config.type === "bar") ? 0 : this.config.styles.innerStrokeWidth);

  this.graph.selectAll(".fg-y-axis .tick")
    .attr("stroke", this.config.styles.innerStrokeColor)
    .attr("stroke-width", this.config.styles.innerStrokeWidth);

  this.graph.selectAll(".fg-x-axis text")
    .attr("stroke", "none")
    .attr("fill", this.config.styles.axes.x.ticks.fillColor)
    .attr("font-size", this.config.styles.axes.x.ticks.fontSize);

  this.graph.selectAll(".fg-y-axis text")
    .attr("stroke", "none")
...
FireGrapher.prototype.draw = function() {
  switch (this.config.type) {
    ...
    case "scatter":
      this.graphData = {};
      this.xDomain = { min: Number.MAX_VALUE/2, max: Number.MIN_VALUE/2 };
      this.yDomain = { min: Number.MAX_VALUE/2, max: Number.MIN_VALUE/2 };
      this.numSeries = 0;
