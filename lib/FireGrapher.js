var FireGrapher = (function() {
  ...
  FireGrapher.prototype.graph = function (cssSelector, firebaseRef, config) {
    ...
    // Clean up the data
    if (typeof this.config.graph === "undefined") {
      this.config.graph = {};
    }
  ...
  FireGrapher.prototype.drawLineGraph = function() {
    ...
    this.graph.append("g")
      ...
      .call(d3.svg.axis()
        .scale(d3.scale.linear().domain([this.config.xCoord.min, this.config.xCoord.max]).range([0, this.config.graph.width]))
    ...
    this.graph.append("g")
      ...
      .call(d3.svg.axis()
        .orient("left")
        .scale(d3.scale.linear().domain([this.config.yCoord.min, this.config.yCoord.max]).range([this.config.graph.height, 0]))
